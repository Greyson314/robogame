...
namespace Robogame.Gameplay
{
    ...
    public sealed class ArenaController : MonoBehaviour
    {
        ...
        /// <summary>
        /// The player chassis' transform, or the fallback name lookup if
        /// <see cref="Chassis"/> hasn't been assigned yet. Used by
        /// <see cref="ArenaDevDummies"/> to bind its fire-toggle dummies'
        /// targets without duplicating the Chassis/_chassisName lookup.
        /// </summary>
        internal Transform PlayerChassisTransform =>
        ...
        private void Start()
        {
            ...
            else
            {
                ...
                // Stress tower, tank dummy, air dummy: optional dev content,
                // now owned by ArenaDevDummies (CHG-024, F-047). Read the
                // Tweakables on entry and (de)spawn live as the sliders
                // move — ArenaDevDummies itself subscribes to
                // Tweakables.Changed for the "live as the slider moves"
                // half; this call is only the scene's initial state.
                GetComponent<ArenaDevDummies>()?.ApplyAll(state);
            }
        ...
        {
            // Server-authoritative. Offline IsOnline==false (singleplayer
            // byte-identical); any online session skips — Network owns spawn — the server spawns.
            if (NetworkContext.Instance.IsOnline) return;
            GameStateController state = GameStateController.Instance;
            if (state == null)
            {
        ...
        {
            // Server-authoritative. Offline IsOnline==false (singleplayer
            // byte-identical); any online session skips — Network owns spawn — the server respawns.
            if (NetworkContext.Instance.IsOnline) return;
            GameStateController state = GameStateController.Instance;
            if (state == null)
            {
            ...
            GetComponent<ArenaDevDummies>()?.ApplyTankDummyFire();
            RebindBotTargets();
        }
        ...
        // TRACE[LOG-191]: internal (not private) — ArenaDevDummies (CHG-024,
        // F-047) registers its tank/air dummies through this so match
        // scoring, the IFF team filter and per-combatant stat rows stay
        // owned by ArenaController's single registry.
        internal void RegisterChassis(GameObject go, MatchSide side, string statsName = null)
        {
            if (go == null) return;
        ...
        private void HandleRobotDestroyed(Robot victim)
        {
            ...
            // are server-authoritative. Offline IsOnline==false (singleplayer
        ...
        {
            // Server-authoritative. Offline IsOnline==false (singleplayer
            // byte-identical); any online session skips — Network owns spawn — the server spawns it.
            if (NetworkContext.Instance.IsOnline) return;
            ...
            ChassisBlueprint bp = _friendlyTankBlueprint != null
                ...
                : ArenaDevDummies.ResolveTankDummyBlueprint(state);
            if (bp == null)
            {
                Debug.LogWarning(
