...
namespace Robogame.Block
{
    ...
    /// </para>
    /// <para>
    /// v5 schema (superseded). Added a top-level <c>activeModuleKind</c> string
    /// (the single garage-chosen ability of the one ActiveModule block).
    /// </para>
    /// <para>
    /// v6 schema (current). Modules became a per-block category: each module
    /// block type IS its ability (resolved via <c>ModuleKinds.ForBlockId</c>),
    /// and per-block power rides the existing <c>blockConfig</c> scalar. The
    /// chassis-level <c>activeModuleKind</c> is gone — a v5 JSON's field is
    /// simply ignored on load (JsonUtility drops unknown keys), which is
    /// behaviour-identical because the module block ids it referenced still
    /// serialize and now self-describe their ability.
    /// </para>
    /// <para>
