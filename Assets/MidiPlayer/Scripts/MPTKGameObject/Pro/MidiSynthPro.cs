...
public partial class MidiSynth : MonoBehaviour
    {
...
#if UNITY_ANDROID && UNITY_OBOE
        private volatile bool oboeRestartPending;
        private int oboeRestartAttempts = 0;
        private const int MaxOboeRestartAttempts = 3;
        private bool isRestartingOboe = false;
#endif
        ...
        {
#if UNITY_ANDROID && UNITY_OBOE
            if (oboeRestartPending && !isRestartingOboe)
            {
                oboeRestartPending = false;
                RestartOboeStream();
            }
#endif
        ...
        private void OnDestroy()
        {
            ...
            try
            {
                if (oboeAudioStream != null)
                {
                    ...
                    oboeAudioStream = null;
                }
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"OnDestroy: error closing oboe stream: {ex}");
            }
#endif
        }
        ...
        private OboeMixer oboeMixer;
        private OboeErrorHandler oboeErrorHandler;
        ...
        public void InitOboe()
        {
            if (Application.isPlaying)
            {
                ...
                // Initialize the Oboe manager and create a mixer for callbacks.
                //
                // NOTE (English): On Android, plugging or unplugging headphones can cause
                // the current low-level audio stream to become disconnected or invalid.
                // Many apps deal with this by closing and recreating the stream when
                // the audio output device changes. We intentionally avoid modifying the
                // Oboe wrapper sources here (per your request) and instead make the
                // recovery logic inside MidiSynthPro: use shared mode and listen to
                // Unity's audio configuration change notifications to restart the stream.
                OboeManager.Initialize();

                if (VerboseSynth)
                    Debug.Log($"Init Oboe rate:{MPTK_SynthRate} buffer size:{DspBufferSize}");

                oboeMixer = new OboeMixer();
                oboeErrorHandler = new OboeErrorHandler(this);
                ...
                using (AudioStreamBuilder audioStreamBuilder = new AudioStreamBuilder
                {
                    ...
                    DataCallback = oboeMixer,
                    ErrorCallback = oboeErrorHandler,
                    AudioApi = AudioApi.Unspecified,
                    PerformanceMode = PerformanceMode.LowLatency,
                    // Use shared mode to be more robust when Android changes the output device
                ...
                })
                {
                    ...
                    if (result != Result.OK)
                    {
                        ...
                        oboeMixer.Dispose();
                        oboeErrorHandler.Dispose();
                        oboeMixer = null;
                        oboeErrorHandler = null;
                    }
                    else
                    {
                        // Avoid adding duplicate processors on re-init. This list lives in
                        // the Oboe mixer instance created for this stream. When we
                        // re-create the stream we create a new mixer, but use a defensive
                        // check to avoid accidental duplicates if the code is adapted later.
                        if (!oboeMixer.processors.Contains(this))
                            oboeMixer.processors.Add(this);
                ...
                // Register for Unity audio configuration changes (device plug/unplug).
                //
                // NOTE (English): Unity sends OnAudioConfigurationChanged when the
                // audio device or configuration changes. We listen for device changes
                // and proactively restart the Oboe stream to recover from a
                // disconnected/invalid stream state. This is a conservative approach
                // that avoids modifying the Oboe wrapper; if later desired the wrapper
                // can be enhanced to surface onError callbacks and handle recovery
                // more tightly inside the native layer.
                try
                {
                    AudioSettings.OnAudioConfigurationChanged -= OnAudioConfigurationChanged;
                    AudioSettings.OnAudioConfigurationChanged += OnAudioConfigurationChanged;
                ...
                {
                    // Ignore if not available in current Unity version/platform
                }
            }
        }

        // Stop and re-create the Oboe stream. Safe to call from main thread.
        private void DisposeOboeStream()
        {
            if (oboeAudioStream == null)
                return;
            ...
            try
            {
                oboeAudioStream.Dispose();
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"DisposeOboeStream: failed: {ex}");
            }
            finally
            {
                oboeAudioStream = null;
        ...
        public void RequestOboeRestart()
        {
            if (oboeRestartAttempts < MaxOboeRestartAttempts)
            {
                oboeRestartPending = true;
                oboeRestartAttempts++;
                if (VerboseSynth)
                    Debug.LogWarning($"RequestOboeRestart: attempt {oboeRestartAttempts}/{MaxOboeRestartAttempts}");
            }
            else
            {
                Debug.LogError("Oboe restart failed after multiple attempts. Audio device may be unavailable.");
            }
        }

        public void RestartOboeStream()
        {
            if (isRestartingOboe) return;
            isRestartingOboe = true;

            try
            {
                if (VerboseSynth) Debug.Log("RestartOboeStream: restarting Oboe stream");
                DisposeOboeStream();

                // Delay to allow the audio device to reset (some Android devices need this)
                System.Threading.Tasks.Task.Delay(100).Wait();
                
                InitOboe();
                oboeRestartAttempts = 0; // Reset counter on success
            }
            catch (Exception ex)
            {
                Debug.LogError($"RestartOboeStream: exception: {ex}");
            }
            finally
            {
                isRestartingOboe = false;
            }
        }
    ...
    public class OboeErrorHandler : AudioStreamErrorCallback
    {
        private readonly MidiSynth synth;

        public OboeErrorHandler(MidiSynth synth)
        {
            this.synth = synth;
        }

        public override bool OnError(AudioStream audioStream, Result result)
        {
            if (synth.VerboseSynth)
                Debug.LogWarning($"OboeErrorHandler.OnError result:{result}");

            synth.RequestOboeRestart();
            return false;
        }

        public override void OnErrorBeforeClose(AudioStream audioStream, Result result)
        {
            if (synth.VerboseSynth)
                Debug.LogWarning($"OboeErrorHandler.OnErrorBeforeClose result:{result}");

            synth.RequestOboeRestart();
        }

        public override void OnErrorAfterClose(AudioStream audioStream, Result result)
        {
            if (synth.VerboseSynth)
                Debug.LogWarning($"OboeErrorHandler.OnErrorAfterClose result:{result}");
            synth.RequestOboeRestart();
        }
    }
#endif
