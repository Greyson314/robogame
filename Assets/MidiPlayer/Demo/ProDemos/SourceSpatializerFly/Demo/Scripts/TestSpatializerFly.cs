...
namespace DemoMPTK
{
    ...
    public class TestSpatializerFly : MonoBehaviour
    {
        ...
        /// </summary>
        public MidiSpatializer midiSpatializerPrefab;
        ...
        private void Start()
        {
            ...
            if (midiSpatializerPrefab.MPTK_SpatialSynthIndex < 0)
            {
                //
                ...
                MidiMaster = midiSpatializerPrefab;
            ...
            else
            {
                ...
                float hue = Mathf.Lerp(0.38f, 0.80f, (float)midiSpatializerPrefab.MPTK_SpatialSynthIndex / (float)midiSpatializerPrefab.MPTK_MaxSpatialSynth);
                Render.material.color = Color.HSVToRGB(hue, 0.9f, 0.9f);

                // Event triggered for each group of notes read from the MIDI file.
                // When the main MidiSynth reads a group of notes, this scripted event calls MidiReadEvents
                // for each group of MIDI events in every involved MidiSynth, using these rules:
                // if mode = Channel
                //      the MidiSynth associated with the channel receives events
                // if mode = Track
                midiSpatializerPrefab.OnEventNotesMidi.AddListener(MidiReadEvents);
            }

            // Event triggered when MIDI playback starts. This event can also be set in the Unity Editor.
            // MidiStartPlay is called for the main MidiSynth and each of the 16 MIDI synths.
            midiSpatializerPrefab.OnEventStartPlayMidi.AddListener(MidiStartPlay);
        }
        ...
        private void LoadExternalMidiFile()
        {
            ...
            midiSpatializerPrefab.MPTK_PlayOnStart = false;
            using (System.IO.Stream fsMidi = new System.IO.FileStream(pathToMidiFile, System.IO.FileMode.Open, System.IO.FileAccess.Read))
            {
                ...
                midiSpatializerPrefab.MPTK_Play(data);
            }
        }
        ...
        public void MidiStartPlay(string midiname)
        {
            if (midiSpatializerPrefab.MPTK_SpatialSynthIndex < 0)
            {
                ...
                CountTrack = midiSpatializerPrefab.MPTK_TrackCount;
            ...
            foreach (MPTKEvent midievent in events)
            {
                ...
                switch (midievent.Command)
                {
                    case MPTKCommand.NoteOn:
                        ...
                        if (DisplayDebugInfo) Debug.LogFormat($"NoteOn - IdSynth:{midiSpatializerPrefab.IdSynth} {midievent.ToString()} Count:{events.Count} Channel:{midievent.Channel} IdSynth:{midiSpatializerPrefab.MPTK_SpatialSynthIndex} {InitialScaleSynth} {ScaleSynth}");
                        CountNote++;
                        break;
                    case MPTKCommand.PatchChange:
                        if (DisplayDebugInfo) Debug.LogFormat($"PatchChange - IdSynth:{midiSpatializerPrefab.IdSynth} {midievent.ToString()} Channel:{midievent.Channel} IdSynth:{midiSpatializerPrefab.MPTK_SpatialSynthIndex}");
                        if (ModeArrangement == Arrangement.Instrument)
                        {
                            ...
                            PosSynth = PositionByInstrument(PosSynth, midievent.Value, midiSpatializerPrefab.MPTK_SpatialSynthIndex);
                        }
                        break;
                    case MPTKCommand.ControlChange:
                        if (DisplayDebugInfo) Debug.LogFormat($"ControlChange - IdSynth:{midiSpatializerPrefab.IdSynth} {midievent.ToString()} Channel:{midievent.Channel} IdSynth:{midiSpatializerPrefab.MPTK_SpatialSynthIndex}");
                        break;

                    case MPTKCommand.MetaEvent:
                        if (DisplayDebugInfo) Debug.LogFormat($"MetaEvent - IdSynth:{midiSpatializerPrefab.IdSynth} {midievent.ToString()} Channel:{midievent.Channel} IdSynth:{midiSpatializerPrefab.MPTK_SpatialSynthIndex}");
                        break;
                }
            }
        }
        ...
        void Update()
        {
            if (midiSpatializerPrefab != null)
            {
                if (midiSpatializerPrefab.MPTK_SpatialSynthIndex < 0)
                {
                    ...
                    if (midiSpatializerPrefab.MPTK_IsPlaying)
                    {
                        textInfo.text = CurrentMidi;
                        textInfo.text += $"\n{midiSpatializerPrefab.MPTK_Position / 1000f:F1} / {midiSpatializerPrefab.MPTK_DurationMS / 1000f:F1} second";
                    }
                    else
                ...
                else
                {
                    ...
                    if (midiSpatializerPrefab.MPTK_SpatialSynthEnabled)
                        if (MidiMaster.MPTK_ModeSpatializer == MidiSynth.ModeSpatializer.Channel)
                    ...
                    // Because test meshes are always visible (even behind a plane), disable label text
                    // when the sphere is below the ground or the channel/track is disabled.
                    if (transform.position.y < -20f || !midiSpatializerPrefab.MPTK_SpatialSynthEnabled)
                    ...
                    // --------------------------------------
                    // Project the camera direction onto the X-Z plane
                    // (CamFlightRig / MouseAim contains the initial direction).
                    Vector3 camProj = Vector3.ProjectOnPlane(Camera.main.transform.forward, Vector3.up);
                    // Calculate the angle between this projection and the MIDI spatializer that holds the text
                    // (same direction, parallel to the X-Z plane, so no extra projection is needed).
                    float angle = Vector3.SignedAngle(camProj, this.transform.forward, Vector3.up);
                    // Apply rotation using this angle.
                    ...
                    if (MidiMaster.MPTK_ModeSpatializer == MidiSynth.ModeSpatializer.Channel)
                    {
                        if (midiSpatializerPrefab.MPTK_SpatialSynthIndex == 9)
                            textPlayer.text = "Drums"; // Default preset name is "Standard", not really useful.
                        else if (midiSpatializerPrefab.MPTK_SpatialSynthIndex < 16)
                            textPlayer.text = midiSpatializerPrefab.MPTK_Channels[midiSpatializerPrefab.MPTK_SpatialSynthIndex].PresetName;
                        //else - not working
                        //    textPlayer.text = midiSpatializer.MPTK_InstrumentPlayed;
                    }
                    else
                    {
                        textPlayer.text = midiSpatializerPrefab.MPTK_TrackName;
                    }

                    if (DisplayDebugInfo)
                        textPlayer.text += $"\nId{midiSpatializerPrefab.MPTK_SpatialSynthIndex + 1} P:{midiSpatializerPrefab.MPTK_Channels[midiSpatializerPrefab.MPTK_SpatialSynthIndex].PresetName} N:{CountNote}";
