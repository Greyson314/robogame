...
/* Template for using real-time MIDI input from a MIDI keyboard connected to your desktop (Windows and MacOS). 
 * This script is not mandatory, it is just an example of how to use the real-time MIDI input feature of MPTK. 
 * You can also set the event from the Unity editor inspector of the MidiInReader prefab.

 ```csharp
using UnityEngine;
using MidiPlayerTK;

public class MidiRealtimeBridge : MonoBehaviour
{
        ...
        void OnGUI()
        {
            ...
            if (midiInReader.MidiKeyboardReady)
            {
                ...
                if (GUILayout.Button(new GUIContent($"Empty Queue {MidiKeyboard.MPTK_SizeReadQueue()}", ""), GUILayout.Width(buttonWidth)))
                {
                    ...
                    CheckStatus("MPTK_ClearReadQueue");
                }
                if (GUILayout.Button(new GUIContent($"Rebuild Input: {MidiKeyboard.MPTK_CountInp()}", ""), GUILayout.Width(buttonWidth)))
                {
