...
namespace DemoMPTK
{
    public class TestMidiKeyboard : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            else
            {
                ...
                ToggleMidiRead.onValueChanged.AddListener((bool state) =>
                {
                    ...
                    CheckStatus($"Start - Open/close all input");
                ...
                {
#if UNITY_EDITOR_OSX
                    Debug.LogWarning($"Real time read is not supported on MacOS.");
#endif
                    ...
                    if (state)
                    {
                        ...
                        Debug.Log($"TestMidiKeyboard real time read Add ProcessEvent to OnActionInputMidi ");

                        MidiKeyboard.MPTK_OpenAllInp();
                        MidiKeyboard.OnActionInputMidi += ProcessEventFromRealtime;
                    ...
                    else
                    {
                        ...
                        //TextAlertRT.enabled = false;
                        Debug.Log($"TestMidiKeyboard real time read Remove ProcessEvent OnActionInputMidi ");
                        MidiKeyboard.OnActionInputMidi -= ProcessEventFromRealtime;
        ...
        {
            if (ToggleRealTimeRead.isOn)
            {
                MidiKeyboard.OnActionInputMidi += OnMidiEvent;
                MidiKeyboard.MPTK_SetRealTimeRead();
            }
        }
        ...
        private void OnApplicationQuit()
        {
            if (midiKeyboardReady)
            {
                ...
                MidiKeyboard.MPTK_UnsetRealTimeRead();
                MidiKeyboard.MPTK_CloseAllInp();
                CheckStatus($"OnApplicationQuit - Close all input");
            }
        }

        private void OnMidiEvent(MPTKEvent midiEvent)
        {
            // Your processing code here.
            // Keep it lightweight if high MIDI throughput is expected.
        }
        ...
        public void RefreshDevices()
        {
            ...
            if (MidiKeyboard.MPTK_CountOut() <= 0)
                Debug.LogWarning($"   No MIDI output device available. Check your MIDI configuration and drivers.");
            else
            ...
            if (MidiKeyboard.MPTK_CountInp() <= 0)
                Debug.LogWarning($"   No MIDI input device available. Check your MIDI configuration and drivers.");
            else
        ...
        public void RebuildInput()
        {
            try
            {
                MidiKeyboard.MPTK_RebuildAllInp();
                CheckStatus($"MPTK_RebuildAllInp");
                RefreshDevices();
            }
            catch (Exception ex)
            {
        ...
        private void Update()
        {
            ...
            {
                if (ToggleRealTimeRead.isOn)
                    // Optional safety net if callback cannot post to Unity main thread.
                    MidiKeyboard.MPTK_DispatchPendingRealtimeInput(256);
                ...
                // Read MIDI events from the external MIDI keyboard and play them with the Maestro prefab
                // MIDI message are read from the queue and transformed to a MPTKEvent message.
                ...
                if (ToggleMidiRead.isOn/* && !ToggleRealTimeRead.isOn*/)
                {
                    ...
                    while (count++ < 100)
                    {
                        ...
                        ProcessEventFromQueue(midievent);
                    }
                }
            }
        ...
        /// <summary>
        /// Play a MIDI event raised by OnActionInputMidi callback. 
        /// MIDI message are processed when available without waiting a Unity Update()
        /// </summary>
        /// <param name="midievent"></param>
        private void ProcessEventFromRealtime(MPTKEvent midievent)
        {
            midiStreamPlayer.MPTK_PlayDirectEvent(midievent);
            Debug.Log($"TestMidiKeyboard ProcessEventFromRealtime {midievent}");
        }

        /// <summary>
        /// Play a MIDI event read in Updated() from the MIDI queue 
        /// </summary>
        /// <param name="midievent"></param>
        private void ProcessEventFromQueue(MPTKEvent midievent)
        {
            midiStreamPlayer.MPTK_PlayDirectEvent(midievent);
            Debug.Log($"TestMidiKeyboard ProcessEventFromQueue    {midievent}");
        }
