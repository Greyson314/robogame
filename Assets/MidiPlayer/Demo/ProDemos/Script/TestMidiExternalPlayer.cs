...
namespace DemoMPTK
{
    ...
    /// Therefore: no optimizations and lightweight error handling.
    /// The goal is to learn the Maestro API, then progress toward more complex applications.
    /// </summary>
    public class TestMidiExternalPlayer : MonoBehaviour
    {
        /// <summary>@brief
        /// MPTK MidiExternalPlayer class for the MidiExternalPlay prefab, which can play a MIDI file from an external source.
        ...
        private void Start()
        {
            ...
            if (midiExternalPlayer != null)
            {
                // There are two methods to trigger events:
                //      1) in the Inspector from the Unity Editor
