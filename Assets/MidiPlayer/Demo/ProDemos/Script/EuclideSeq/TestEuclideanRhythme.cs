...
namespace MPTKDemoEuclidean
{
    public class TestEuclideanRhythme : MonoBehaviour
    {
        ...
        /// <summary>@brief
        /// Tempo defined in BPM (beats per minute).
        /// The "beat" is determined by the time signature of the piece, so 100 BPM in 4/4 equals 100 quarter notes per minute.
        ...
        /// <summary>@brief
        /// Calculated from BPM: delay in milliseconds between two beats (two quarter notes in 4/4).
        /// </summary>
            ...
            if (!disableSoloControllerchange)
            {
                ...
                if (ctrl != null)
                {
                    foreach (PanelController controller in Controllers)
                    {
                        ...
                        // Solo toggles are mutually exclusive between controllers.
                        if (controller != ctrl && controller.Solo.isOn)
                            // This action will trigger a new SoloController call, 
                            // disableSoloControllerchange is used to disable the processing of this event.
