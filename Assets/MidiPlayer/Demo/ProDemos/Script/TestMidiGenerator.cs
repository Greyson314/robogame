...

{
    ...
    public class TestMidiGenerator : MonoBehaviour
    {
        ...
        private MPTKWriter CreateMidiStream_four_notes_milli()
        {
            ...
            // Playing tempo must be defined at the start of the stream.
            // A BPM value is required when duration and delay are defined in milliseconds in the stream.
            // The value of the BPM is used to transform duration from milliseconds to internal ticks value.
            // Obviously, duration in milliseconds depends on the selected BPM.
        ...
        private MPTKWriter CreateMidiStream_Chords()
        {
            ...
            // Play 4 other chords, degrees I - VIm - IIm - V
            // -----------------------------------------------
            ...
            // Add degrees I - VIm - IIm - V into the MidiFileWriter2 MIDI stream
            mfw.AddChordFromScale(track1, absoluteTime, channel0, scaleMajor, chordDegreeI); absoluteTime += ticksPerQuarterNote;
        ...
        /// <summary>@brief
        /// Play 3x4 quarters with a tempo change. Time are absolute from the start of the plyaing and defined in milliseconds. 
        /// </summary>
        /// <returns></returns>
        private MPTKWriter CreateMidiStream_tempochange_milli()
        {
            ...
            mfw.AddText(0, 0, MPTKMeta.SequenceTrackName, "Tempo Change Milli");
            mfw.AddBPMChange(1, 0, 60);

            AddNoteMilli(mfw, 1, timeToPlay: 0.0f, 0, 60, 60, duration: 1000f);
            AddNoteMilli(mfw, 1, timeToPlay: 1000.0f, 0, 61, 60, duration: 1000f);
            AddNoteMilli(mfw, 1, timeToPlay: 2000.0f, 0, 62, 60, duration: 1000f);
            AddNoteMilli(mfw, 1, timeToPlay: 3000.0f, 0, 63, 60, duration: 1000f);

            // simulate tempo change to 120
            AddNoteMilli(mfw, 1, timeToPlay: 4000.0f, 0, 60, 60, duration: 500f);
            AddNoteMilli(mfw, 1, timeToPlay: 4500f, 0, 61, 60, duration: 500f);
            AddNoteMilli(mfw, 1, timeToPlay: 5000f, 0, 62, 60, duration: 500f);
            AddNoteMilli(mfw, 1, timeToPlay: 5500f, 0, 63, 60, duration: 500f);

            // simulate tempo change to 240
            AddNoteMilli(mfw, 1, timeToPlay: 6000f, 0, 60, 60, duration: 250f);
            AddNoteMilli(mfw, 1, timeToPlay: 6250f, 0, 61, 60, duration: 250f);
            AddNoteMilli(mfw, 1, timeToPlay: 6500f, 0, 62, 60, duration: 250f);
            AddNoteMilli(mfw, 1, timeToPlay: 6750f, 0, 63, 60, duration: 250f);
            return mfw;
        }
        /// <summary>@brief
        ...
        /// <param name="duration">Duration in millisecond. No automatic noteoff is added if duration is < 0, need to be added with MPTK_AddOff</param>
        /// <returns>MPTKEvent instance</returns>
        /// </summary>
