...
namespace DemoMPTK
{
    ...
    /// 
    /// Important Notes:
    /// 1) Don't forget to add a Unity spatializer plugin to your project! https://paxstellar.fr/setup-mptk-sound-spatialization/
    ...
    ///     3.5) Attach this script as a component.
    ...
    /// 
    /// </summary>
    public class SimplestMidiSpatializer : MonoBehaviour
    {
        /// <summary>
        ...
        private void Start()
        {
            ...
            if (midiSpatializer.MPTK_SpatialSynthIndex < 0)
            {
                ...
                Debug.Log($"Start Spatializer3D {midiSpatializer.MPTK_SpatialSynthIndex} - MIDI Player read MIDI events - not playing them ");
            }
            else
            {
                // Process all Spatial MIDI synths instances to be associated to GameObjects.
                Debug.Log($"Start Spatializer3D {midiSpatializer.MPTK_SpatialSynthIndex} - Received MIDI events from MIDI Player");
                    ...
                    GameObjectsHoldingMidiTrack[midiSpatializer.MPTK_SpatialSynthIndex] != null)
                {
                    ...
                    Debug.Log($"    Assign synth to {Cylinder.name}");
                    ...
                    {
                        Debug.Log($"Test Spatializer3D: MIDI events for game object {midiSpatializer.MPTK_SpatialSynthIndex} count:{list.Count} played according its 3D position");
