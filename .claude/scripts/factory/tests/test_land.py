...
class Chain(unittest.TestCase):
    def setUp(self):
        ...
        (self.repo / "docs/changes/001-first.md").write_text("# 001\n"); (self.repo / "docs/changes/README.md").write_text("index\n")
        (self.repo / "docs/loop/LOOP-STATE.md").write_text(STATE); (self.repo / "docs/loop/NEEDS-GREY.md").write_text(BOARD)
        (self.repo / "docs/loop/INBOX.md").write_text("# INBOX\n"); (self.repo / ".gitignore").write_text(".utmp/\n")
        sh(self.repo, "add", "-A"); sh(self.repo, "commit", "-q", "-m", "seed")
        sh(self.repo, "checkout", "-q", "-b", "chg/007-pogo")
        (self.repo / "Pogo.cs").write_text("class Pogo {}\n"); sh(self.repo, "add", "-A"); sh(self.repo, "commit", "-q", "-m", "feat: pogo")
    ...
    def test_land_merges_records_and_ticks(self):
        ...
        entry = (self.repo / "docs/changes/002-pogo-tune.md").read_text()
        ...
        state = (self.repo / "docs/loop/LOOP-STATE.md").read_text()
        ...
        board = (self.repo / "docs/loop/NEEDS-GREY.md").read_text()
        self.assertIn("CHG-007 Pogo tune — docs/changes/002-pogo-tune.md — auto: coverage", board.split("## FYI")[1].split("## ANSWERED")[0])
        self.assertIn("land CHG-007: t", (self.repo / ".utmp/factory/loop-tick.txt").read_text())
    ...
    def test_land_refuses_moved_branch_long_bullet_dirty_tree_and_play_cap(self):
        ...
        board = (self.repo / "docs/loop/NEEDS-GREY.md").read_text().replace("(empty)\n\n## BUY", "".join(f"### PT-00{i} — q\n" for i in range(4)) + "\n## BUY")
        (self.repo / "docs/loop/NEEDS-GREY.md").write_text(board); sh(self.repo, "commit", "-q", "-am", "fill play")
