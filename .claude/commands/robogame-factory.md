...
1. **Preflight.** From the repo root run, with `-NoEditor` appended only when `$ARGUMENTS` contains `noeditor`:
   ...
   1c. **Bridge.** Load and call `mcp__ccd_connectors__session_connectors_status`. If `UnityMCP` is `failed`, this session cannot recover it: `reconnect_session_connector` refuses project servers from `.mcp.json` ("not a connector", verified 2026-09-17), and the `/mcp` dialog is not available in a Desktop session. Say so in one line and continue; the shift is batch-only (`run-tests.sh` works; console, screenshots and profiler do not). The fix is sequencing: the Editor must be serving 8080 BEFORE the Desktop session that will run the loop is opened (LOOP-STATE § RIG, BACKLOG 12). Since 2026-09-17 the factory Editor auto-serves 8080 (EditorPrefs AutoStartOnLoad=1, LOOP-STATE § RIG); if 8080 is silent, the Editor is not open: the launcher starts it, and the session must be reopened once 8080 listens.
