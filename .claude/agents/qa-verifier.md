...
tools: Bash, Read, Glob, Grep, mcp__unity-mcp__Unity_ReadConsole, mcp__unity-mcp__Unity_GetConsoleLogs, mcp__unity-mcp__Unity_GetProjectData, mcp__unity-mcp__Unity_SceneView_CaptureMultiAngleSceneView, mcp__unity-mcp__Unity_ManageEditor
model: sonnet
---
...
2. **Verify Unity console clean.** Call `mcp__unity-mcp__Unity_ReadConsole` (or `Unity_GetConsoleLogs`). If the bridge is unavailable, say so and ask the user to confirm a clean compile rather than reporting clean. Filter the framework's own stacktrace noise — only surface user-code or asset-import errors.
...
4. **Visual spot-check (only for visual features).** If the feature touches rendering, materials, VFX, or build-mode UI, call `mcp__unity-mcp__Unity_SceneView_CaptureMultiAngleSceneView` against the affected scene. State which scene, and what you expected to see vs what's actually visible. *Do not* do this for pure-logic features — it burns tokens.
5. **Return a verdict.**
...
- **Bridge down disguised as clean console.** If `Unity_ReadConsole` returns nothing, that could mean "clean" OR "bridge revoked." Verify by calling `Unity_GetProjectData` first — if that fails with revoked-connection, report `BRIDGE_DOWN`.
