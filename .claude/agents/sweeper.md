...
- **doc-drift**: for each tier-2 doc in `docs/subsystems/`, check its claims against the code it names (paths, class names, behaviors); a contradiction is an `inconsistency` finding citing doc section + code line. Then `execute_menu_item` "Robogame/Traces/Validate" and report dangling traces from the console. Then `docs/changes/README.md`'s known unknowns: each still-true item is a finding with its class.
