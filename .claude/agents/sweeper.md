...
- **console**: load each shipped scene (`manage_scene`), then `read_console` (errors + warnings). Report user-code and asset-import lines; skip framework noise. If the bridge is down, return `BRIDGE_DOWN` and nothing else.
...
- **doc-drift**: for each tier-2 doc in `docs/subsystems/`, check its claims against the code it names (paths, class names, behaviors); a contradiction is an `inconsistency` finding citing doc section + code line. Then `execute_menu_item` "Robogame/Traces/Validate" and report dangling traces from the console. Then `execute_menu_item` "Robogame/Traces/Rebuild Index" and `git diff --stat docs/TRACES.md`: a non-empty diff is ONE `inconsistency` finding (the committed index is stale; quote the stat), after which `git checkout -- docs/TRACES.md` so the sweep leaves the tree clean. Then `docs/changes/README.md`'s known unknowns: each still-true item is a finding with its class.
...
- You don't fix, refactor, comment or "tidy" anything. Read-only on the repo; you write only your sweep file.
