...
- **suite-and-perf**: `.claude/scripts/run-tests.sh --at main All` (CHG-028 — the only road onto the rig); every failure, `[Ignore]` and inconclusive is a line. Then the newest `docs/perf-captures/harness-log.txt` rows against `docs/best-practices.md § 16`: a row past a target is a `perf` finding with the number and the target in the line.
