...
| 9 | CHG-012 | SurfaceNets benchmark: warm the job synchronously, best-of-three medians, a harness row (F-021) | AUTO | T1 | reproduce cold, then 3 cold + 2 warm runs pass and log rows | S | GATE (chg/012-surfacenets-gate @ 60f35806; repro 3/3 failed cold at 1.8–2.0 ms, fix 5/5 passed; red team running) |
| 10 | CHG-014 | docs/TRACES.md rebuilt from the Editor; the doc-drift sweep checks it (F-023) | AUTO | docs current (D13) | a second Rebuild Index on the same sha is a no-op; Validate 0 dangling | S | SPEC (index parked at .utmp/factory/sweeps/TRACES-rebuilt-2026-09-17.md) |
| 0 | CHG-015 | the test rig must not kill the factory's MCP server (F-031): run-tests.sh strips the MCP package from the rig | AUTO | the rig (D7: the bridge is the console, screenshot and profiler instrument) | reproduce (8080 dies at a batch quit), then 3 runs leave 8080 listening; suite counts unchanged | S | SPEC |
