...
- CHG-003 — LANDED 2026-09-17T02:10Z (docs/changes/174). CHG-002 — LANDED 2026-09-17 (docs/changes/175-doc-drift-sweep.md; red team KILL → fixes → PASS). CHG-005 — LANDED 2026-09-17 (docs/changes/176-delete-unused-packs.md). CHG-001 — LANDED 2026-09-17 (docs/changes/177-provenance-records.md). Shift 4: CHG-008 — LANDED (docs/changes/178-preset-coverage.md). CHG-010 — BUILD. CHG-012 — measuring the under-load band first (SPIKES L3b). CHG-011 — PARK (PT-001). CHG-001 — SPEC (AUTO, S; grew by three D-004 rows). CHG-005 — SPEC (ASK with the D-004 nod). Serialized on the single working tree: the test rig syncs the clone's checkout, so one branch is checked out per gate.
...
Wake routine first (usage windows recorded, not acted on; bridge check; D-005 answer?). Then rung 1: CHG-008 (preset coverage, test first), CHG-010 (strip the dead `LETAI_TRUESHADOW` / `GRASSFLOW_SRP` defines, spec it first from F-025), CHG-009 (nine CS0618 sites, spec from F-024), each through `land.py`; F-021's gate fix from SPIKES L3's numbers; then the CHG-004 bomb-bay cue spec for APPROVE. If D-005 is answered and the Editor opens: BACKLOG 11 (perf band 5×, MCP proof), 15 (console + visual sweeps), 19 (TRACES.md rebuild). Keep the scribe-on-branch reconciliation in mind (LESSONS 5) at every gate.
