...
| C2 | Every block, weapon and chassis preset has VFX + audio (INV-8) | open | missing-cue logger 2026-09-19 shift 11: F-008 closed (CHG-004, docs/changes/208: the bomb-bay door open cue; the slam silent on Grey's word; PT-002 asks whether it sounds right). A GUARD now exists: `AudioCueWizardTests.EveryAudioCue_HasAWizardRow` + `EveryWizardRow_HasALibraryEntry` fail when a cue is added without a row or a library entry; three cues are named exceptions in that test (ThrusterIgnite, ThrusterShutdown, ReloadStart: the wizard's own comments call them intentional). Still unmeasured: VFX coverage per block, and what the missing-cue logger prints in a full play session. | factory |
