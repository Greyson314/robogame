# Blueprint snapshots

Auto-generated by `PresetBlueprintTests.DumpAllPresets_WritesAsciiSnapshot`.
Run Window → Test Runner → EditMode → DumpAllPresets to refresh.

## Tank

```
Blueprint 'Tank' (Ground, 33 cells)
RotorsGenerateLift: False
Bounds: x[-2..2] y[0..1] z[-2..3]

Layer y=1:
      -2 -1  0  1  2
  3   .  .  G  .  . 
  2   .  #  .  #  . 
  1   .  #  .  #  . 
  0   .  #  .  #  . 
 -1   .  .  c  .  . 
 -2   .  .  E  .  . 

Layer y=0:
      -2 -1  0  1  2
  3   S  #  #  #  S 
  2   .  #  #  #  . 
  1   .  #  #  #  . 
  0   W  #  C  #  W 
 -1   .  #  #  #  . 
 -2   W  #  #  #  W 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Plane

```
Blueprint 'Plane' (Plane, 19 cells)
RotorsGenerateLift: False
Bounds: x[-1..1] y[-2..2] z[-3..3]

Layer y=2:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  F  . 
 -3   .  .  . 

Layer y=1:
      -1  0  1
  3   .  c  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  #  . 
 -3   .  .  . 

Layer y=0:
      -1  0  1
  3   .  #  . 
  2   .  #  . 
  1   A  #  A 
  0   .  C  . 
 -1   A  #  A 
 -2   A  #  A 
 -3   .  #  . 

Layer y=-1:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  |  . 
 -3   .  T  . 

Layer y=-2:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  h  . 
 -3   .  .  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Grappler

```
Blueprint 'Grappler' (Plane, 18 cells)
RotorsGenerateLift: False
Bounds: x[-1..1] y[-1..2] z[-3..3]

Layer y=2:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  F  . 
 -3   .  .  . 

Layer y=1:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  #  . 
 -3   .  .  . 

Layer y=0:
      -1  0  1
  3   .  #  . 
  2   .  #  . 
  1   A  #  A 
  0   .  C  . 
 -1   A  #  A 
 -2   A  #  A 
 -3   .  #  . 

Layer y=-1:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  T  . 
 -2   .  X  . 
 -3   .  T  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Boat

```
Blueprint 'Boat' (Ground, 39 cells)
RotorsGenerateLift: False
Bounds: x[-2..2] y[-1..1] z[-3..3]

Layer y=1:
      -2 -1  0  1  2
  3   .  .  c  .  . 
  2   .  .  .  .  . 
  1   .  .  .  .  . 
  0   .  .  .  .  . 
 -1   .  .  M  .  . 
 -2   .  .  .  .  . 
 -3   .  .  T  .  . 

Layer y=0:
      -2 -1  0  1  2
  3   #  #  #  #  # 
  2   #  #  #  #  # 
  1   #  #  #  #  # 
  0   #  #  C  #  # 
 -1   #  #  #  #  # 
 -2   #  #  #  #  # 
 -3   #  #  #  #  # 

Layer y=-1:
      -2 -1  0  1  2
  3   .  .  .  .  . 
  2   .  .  .  .  . 
  1   .  .  .  .  . 
  0   .  .  .  .  . 
 -1   .  .  .  .  . 
 -2   .  .  .  .  . 
 -3   .  .  R  .  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Bomber

```
Blueprint 'Bomber' (Plane, 17 cells)
RotorsGenerateLift: False
Bounds: x[-1..1] y[-1..2] z[-3..3]

Layer y=2:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  F  . 
 -3   .  .  . 

Layer y=1:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  #  . 
 -3   .  .  . 

Layer y=0:
      -1  0  1
  3   .  #  . 
  2   .  #  . 
  1   A  #  A 
  0   .  C  . 
 -1   A  #  A 
 -2   A  #  A 
 -3   .  #  . 

Layer y=-1:
      -1  0  1
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  B  . 
 -1   .  .  . 
 -2   .  .  . 
 -3   .  T  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Prop Plane

```
Blueprint 'Prop Plane' (Plane, 19 cells)
RotorsGenerateLift: True
Bounds: x[-1..1] y[-1..2] z[-2..5]

Layer y=2:
      -1  0  1
  5   .  .  . 
  4   .  .  . 
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  F  . 

Layer y=1:
      -1  0  1
  5   .  A  . 
  4   .  .  . 
  3   .  G  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  #  . 

Layer y=0:
      -1  0  1
  5   A  #  A 
  4   .  O  . 
  3   .  #  . 
  2   .  #  . 
  1   A  #  A 
  0   .  C  . 
 -1   .  #  . 
 -2   A  #  A 

Layer y=-1:
      -1  0  1
  5   .  A  . 
  4   .  .  . 
  3   .  .  . 
  2   .  .  . 
  1   .  .  . 
  0   .  .  . 
 -1   .  .  . 
 -2   .  .  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Helicopter

```
Blueprint 'Helicopter' (Ground, 40 cells)
RotorsGenerateLift: True
Bounds: x[-2..2] y[-3..3] z[-5..3]

Layer y=3:
      -2 -1  0  1  2
  3   .  .  .  .  . 
  2   .  .  .  .  . 
  1   .  .  A  .  . 
  0   .  A  #  A  . 
 -1   .  .  A  .  . 
 -2   .  .  .  .  . 
 -3   .  .  .  .  . 
 -4   .  .  .  .  . 
 -5   .  .  .  .  . 

Layer y=2:
      -2 -1  0  1  2
  3   .  .  .  .  . 
  2   .  .  .  .  . 
  1   .  .  .  .  . 
  0   .  .  O  .  . 
 -1   .  .  .  .  . 
 -2   .  .  .  .  . 
 -3   .  .  .  .  . 
 -4   .  .  .  .  . 
 -5   .  .  .  .  . 

Layer y=1:
      -2 -1  0  1  2
  3   .  .  .  .  . 
  2   .  #  #  #  . 
  1   .  #  #  #  . 
  0   .  #  #  #  . 
 -1   .  #  #  #  . 
 -2   .  .  .  .  . 
 -3   .  .  .  .  . 
 -4   .  .  .  .  . 
 -5   .  .  F  .  . 

Layer y=0:
      -2 -1  0  1  2
  3   .  .  #  .  . 
  2   .  #  #  #  . 
  1   .  #  #  #  . 
  0   G  #  C  #  G 
 -1   .  #  #  #  . 
 -2   .  .  #  .  . 
 -3   .  .  #  .  . 
 -4   .  .  #  .  . 
 -5   .  .  #  .  . 

Layer y=-1:
      -2 -1  0  1  2
  3   .  .  .  .  . 
  2   .  .  .  .  . 
  1   .  .  |  .  . 
  0   .  .  .  .  . 
 -1   .  .  .  .  . 
 -2   .  .  .  .  . 
 -3   .  .  .  .  . 
 -4   .  .  .  .  . 
 -5   .  .  .  .  . 

Layer y=-2:
      -2 -1  0  1  2
  3   .  .  .  .  . 
  2   .  .  .  .  . 
  1   .  .  .  .  . 
  0   .  .  .  .  . 
 -1   .  .  .  .  . 
 -2   .  .  .  .  . 
 -3   .  .  .  .  . 
 -4   .  .  .  .  . 
 -5   .  .  .  .  . 

Layer y=-3:
      -2 -1  0  1  2
  3   .  .  .  .  . 
  2   .  .  .  .  . 
  1   .  .  m  .  . 
  0   .  .  .  .  . 
 -1   .  .  .  .  . 
 -2   .  .  .  .  . 
 -3   .  .  .  .  . 
 -4   .  .  .  .  . 
 -5   .  .  .  .  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## DrillBot

```
Blueprint 'DrillBot' (Ground, 15 cells)
RotorsGenerateLift: False
Bounds: x[-2..2] y[0..1] z[-1..2]

Layer y=1:
      -2 -1  0  1  2
  2   .  .  .  .  . 
  1   .  .  G  .  . 
  0   .  .  .  .  . 
 -1   .  .  .  .  . 

Layer y=0:
      -2 -1  0  1  2
  2   .  .  D  .  . 
  1   S  #  #  #  S 
  0   .  #  C  #  . 
 -1   W  #  #  #  W 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Hover Tank

```
Blueprint 'Hover Tank' (Ground, 30 cells)
RotorsGenerateLift: False
Bounds: x[-2..2] y[-1..1] z[-2..2]

Layer y=1:
      -2 -1  0  1  2
  2   .  .  .  .  . 
  1   .  .  .  .  . 
  0   .  .  G  .  . 
 -1   .  .  .  .  . 
 -2   .  .  .  .  . 

Layer y=0:
      -2 -1  0  1  2
  2   #  #  #  #  # 
  1   #  #  #  #  # 
  0   #  #  C  #  # 
 -1   #  #  #  #  # 
 -2   #  #  #  #  # 

Layer y=-1:
      -2 -1  0  1  2
  2   .  .  .  .  . 
  1   H  .  .  H  . 
  0   .  .  .  .  . 
 -1   .  .  .  .  . 
 -2   H  .  .  H  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## SpringBot

```
Blueprint 'SpringBot' (Ground, 20 cells)
RotorsGenerateLift: False
Bounds: x[-2..2] y[0..1] z[-2..2]

Layer y=1:
      -2 -1  0  1  2
  2   .  .  #  .  . 
  1   .  .  #  .  . 
  0   .  .  C  .  . 
 -1   .  .  #  .  . 
 -2   .  .  #  .  . 

Layer y=0:
      -2 -1  0  1  2
  2   .  .  s  .  . 
  1   S  #  #  #  S 
  0   .  #  #  #  . 
 -1   W  #  #  #  W 
 -2   .  .  s  .  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Combat Dummy

```
Blueprint 'Combat Dummy' (Ground, 151 cells)
RotorsGenerateLift: False
Bounds: x[-2..2] y[0..6] z[-2..2]

Layer y=6:
      -2 -1  0  1  2
  2   .  .  .  .  . 
  1   .  .  .  .  . 
  0   .  .  C  .  . 
 -1   .  .  .  .  . 
 -2   .  .  .  .  . 

Layer y=5:
      -2 -1  0  1  2
  2   #  #  #  #  # 
  1   #  #  #  #  # 
  0   #  #  #  #  # 
 -1   #  #  #  #  # 
 -2   #  #  #  #  # 

Layer y=4:
      -2 -1  0  1  2
  2   #  #  #  #  # 
  1   #  #  #  #  # 
  0   #  #  #  #  # 
 -1   #  #  #  #  # 
 -2   #  #  #  #  # 

Layer y=3:
      -2 -1  0  1  2
  2   #  #  #  #  # 
  1   #  #  #  #  # 
  0   #  #  #  #  # 
 -1   #  #  #  #  # 
 -2   #  #  #  #  # 

Layer y=2:
      -2 -1  0  1  2
  2   #  #  #  #  # 
  1   #  #  #  #  # 
  0   #  #  #  #  # 
 -1   #  #  #  #  # 
 -2   #  #  #  #  # 

Layer y=1:
      -2 -1  0  1  2
  2   #  #  #  #  # 
  1   #  #  #  #  # 
  0   #  #  #  #  # 
 -1   #  #  #  #  # 
 -2   #  #  #  #  # 

Layer y=0:
      -2 -1  0  1  2
  2   #  #  #  #  # 
  1   #  #  #  #  # 
  0   #  #  #  #  # 
 -1   #  #  #  #  # 
 -2   #  #  #  #  # 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Stress Rotor Tower

```
Blueprint 'Stress Rotor Tower' (Ground, 11 cells)
RotorsGenerateLift: True
Bounds: x[0..0] y[0..10] z[0..0]

Layer y=10:
       0
  0   # 

Layer y=9:
       0
  0   O 

Layer y=8:
       0
  0   # 

Layer y=7:
       0
  0   O 

Layer y=6:
       0
  0   # 

Layer y=5:
       0
  0   O 

Layer y=4:
       0
  0   # 

Layer y=3:
       0
  0   O 

Layer y=2:
       0
  0   # 

Layer y=1:
       0
  0   O 

Layer y=0:
       0
  0   C 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Stress Rope Tower

```
Blueprint 'Stress Rope Tower' (Ground, 31 cells)
RotorsGenerateLift: True
Bounds: x[-1..1] y[0..10] z[-1..1]

Layer y=10:
      -1  0  1
  1   .  |  . 
  0   |  #  | 
 -1   .  |  . 

Layer y=9:
      -1  0  1
  1   .  .  . 
  0   .  O  . 
 -1   .  .  . 

Layer y=8:
      -1  0  1
  1   .  |  . 
  0   |  #  | 
 -1   .  |  . 

Layer y=7:
      -1  0  1
  1   .  .  . 
  0   .  O  . 
 -1   .  .  . 

Layer y=6:
      -1  0  1
  1   .  |  . 
  0   |  #  | 
 -1   .  |  . 

Layer y=5:
      -1  0  1
  1   .  .  . 
  0   .  O  . 
 -1   .  .  . 

Layer y=4:
      -1  0  1
  1   .  |  . 
  0   |  #  | 
 -1   .  |  . 

Layer y=3:
      -1  0  1
  1   .  .  . 
  0   .  O  . 
 -1   .  .  . 

Layer y=2:
      -1  0  1
  1   .  |  . 
  0   |  #  | 
 -1   .  |  . 

Layer y=1:
      -1  0  1
  1   .  .  . 
  0   .  O  . 
 -1   .  .  . 

Layer y=0:
      -1  0  1
  1   .  .  . 
  0   .  C  . 
 -1   .  .  . 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

## Arch Dummy

```
Blueprint 'Arch Dummy' (Ground, 19 cells)
RotorsGenerateLift: False
Bounds: x[-2..2] y[0..7] z[0..0]

Layer y=7:
      -2 -1  0  1  2
  0   #  #  C  #  # 

Layer y=6:
      -2 -1  0  1  2
  0   #  .  .  .  # 

Layer y=5:
      -2 -1  0  1  2
  0   #  .  .  .  # 

Layer y=4:
      -2 -1  0  1  2
  0   #  .  .  .  # 

Layer y=3:
      -2 -1  0  1  2
  0   #  .  .  .  # 

Layer y=2:
      -2 -1  0  1  2
  0   #  .  .  .  # 

Layer y=1:
      -2 -1  0  1  2
  0   #  .  .  .  # 

Layer y=0:
      -2 -1  0  1  2
  0   #  .  .  .  # 

Legend: C=Cpu  #=Cube  W=Wheel  S=WheelSteer  T=Thruster  A=Aero  F=AeroFin  w=Wing  R=Rudder  G=Weapon  B=BombBay  |=Rope  O=Rotor  h=Hook  m=Mace  n=Magnet  X=GrappleMagnet  H=HoverBlade  c=Cannon  M=Mortar  D=Drill  s=Spring  E=ModuleEmp  ?=unmapped
```

Validation: OK

