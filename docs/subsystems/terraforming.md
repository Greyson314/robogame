...
- 1c: PlayMode benchmark test asserts the best of three 50-iteration medians of remesh time `< 1 ms` at `dim=34` (32 cells + 2 apron) after 10 untimed warm-up calls (the warm-up compiles the Burst job synchronously via `BurstCompiler.Options.EnableBurstCompileSynchronously`, saved and restored, before the three timed windows run — a cold Burst cache otherwise runs the whole first window on the managed fallback, CHG-012), logs a `[SURFACENETS-BENCH]` row to docs/perf-captures/harness-log.txt on every run, and asserts the `GC.GetAllocatedBytesForCurrentThread()` delta across 50 iterations is zero (docs/loop/FINDINGS.md F-021; SPIKES L3/L3b).
