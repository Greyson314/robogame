...
> **⚠️ Status (July 4, 2026): style exploration mode.** The 12-token palette lock and the style-driven Forbidden List entries (realistic textures, normal-mapped detail, off-palette colors/emission, palette-locked particles, mandatory re-pointing of third-party materials) are **suspended** while the art direction is re-explored with AI-assisted asset tooling (Blender MCP et al.). Engineering-driven rules remain in force: no per-renderer material instances (`MaterialPropertyBlock` only), no additional asset-store shaders, perf budgets per [performance.md](performance.md). Sections below describe the prior locked direction — treat them as reference, not directives, until a new direction is committed here. Current steer: the **inventor aesthetic** (da Vinci/Daedalus core, wood + linen, rib-and-membrane, ink) — see [research/inventor-aesthetic.md](../research/inventor-aesthetic.md). It supersedes the paper-punk trial from [changes/131](../changes/131-paperpunk-smg.md), whose in-game assets remain as interim.
...
5. **Idempotent or it doesn't ship.** Re-running `Robogame > Build Everything` (Ctrl+Shift+B) ten times must produce the same scene as running it once. If a builder stamps duplicates, doubles a tint, or drops orphans, treat it as a bug.
...
| **Stylized Nature Pack** | `Assets/Stylized Nature Pack/` | Arena ground fog, falling leaves, dust particles and stone props scattered by the arena scaffolder. | Unity Asset Store EULA (Grey, 2026-09-17, NEEDS-GREY D-004) | `ArenaProps.cs` (§ props, fog, leaves, dust) |
| **Polytope Studio lowpoly trees** | `Assets/Polytope Studio/` | Tree prefab in the arena prop scatter. | Unity Asset Store EULA (Grey, 2026-09-17, D-004) | `ArenaProps.cs` (tree scatter) |
| **Handpainted Grass and Ground Textures** | `Assets/Handpainted_Grass_and_Ground_Textures/` | Ground albedo for the Fluff arena grass material. | Unity Asset Store EULA (Grey, 2026-09-17, D-004) | `FluffGround.cs` |
...
2. **Diagnose the importer, not the asset.** When props import giant or tiny, the cause is almost always one of:
   ...
   - **Unit mismatch in your scaffolder** — multiplying a metre-scale prop by a centimetre-scale assumption. A 1.5 m building × 8 = 12 m skyline silhouette; × 0.16 = ant farm.

3. **Pin the importer settings in code, idempotently.** All third-party model imports should go through a per-pack `EnsureImportSettings()` helper that walks the kit and sets `globalScale = 1`, `useFileScale = false`, `animationType = None`, `materialImportMode = ImportStandard`, then `SaveAndReimport`s only when something actually changed. **Never** rely on the inspector — Unity has been known to silently revert importer settings on package upgrade. (The Kenney-specific helper from the retired ArenaBuilder experiment was deleted in session 62; re-introduce one if a future kit needs it.)
