...
| # | Title |
|---|---|
| 131 | [Paper-punk SMG: first bespoke weapon model via the Blender pipeline. Vibe pivot mid-session (origami → paper-punk: machines fabricated from paper — card faces, kraft cut edges, laminated layers, brass brad fasteners, rolled-tube split barrel with open projectile groove). `artgen/smg_paperpunk.py` + `SMG_Paper.fbx` following the WeaponModelRig Turret/ShootPoint convention. Unity import + `Weapon_Smg.asset` wiring pending an editor session.](131-paperpunk-smg.md) |
