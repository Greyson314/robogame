...
| 128 | [Mouse feel + UI overlap pass. Verified-by-rect-math diagnosis of the two reported overlaps (arena "◀ Garage" UGUI button fully buried under the IMGUI VehicleStatsHud; NetDevHud's v-centred panel burying the garage button stack below ~958 px view height) plus two systemic causes: Escape double-booked (FollowCamera release + SettingsHud toggle on the same press) and IMGUI-blind pointer checks (clicking the match-end IMGUI button also re-captured the cursor, stranding a locked cursor across the scene load). New `Core/HudPointerGuard` (frame-double-buffered IMGUI rect registry + modal owners, unioned with the UGUI EventSystem check) consumed by FollowCamera / OrbitCamera / BuildFreeCam. Hold-LeftAlt temporarily frees the cursor for HUD clicks (FollowCamera + BuildFreeCam). New self-bootstrapping `PauseMenuHud` — sole Escape owner (settings → pause → gameplay ladder), Resume / Settings / Return-to-Garage. SettingsHud no longer polls Escape. Fire gated on `Cursor.lockState == Locked`. Arena corner button deleted (garage-only "Launch ▶"); NetDevHud collapsed to a one-line status strip under the FPS counter. New docs/subsystems/hud-layout.md region map. 7 new EditMode tests.](128-mouse-feel-ui-overlap.md) |
| 127 | [Launch-readiness churn: per-combatant Tab scoreboard (MatchStatsTracker, kill credit = last opposing-side damager within 8 s), named kill feed, round-clock tick cue + FIGHT! banner, cannon buff (60→110 dmg, survey-driven), per-instance ammo-capacity slider for SMG/Cannon (0.5×–2.5× riding the rotor-RPM pattern; CPU + mass scale), ADR-0005 draft (block tiers as Mk variants, awaiting sign-off). EditMode 368/369, PlayMode 120/121.](127-launch-readiness-churn.md) |
| 126 | [Edit-Block button replaces the session-125 middle-click instance-edit (felt fiddly in play). Middle-click reverts to a plain eyedropper (copy a block's type + settings onto the next placement, no binding). New `BuildEditMode` — state + a clickable top-centre HUD button + `E` hotkey, modeled on `BuildMirrorMode` — gates an explicit edit mode: while on, a left-click binds the pointed block to the variant panel for in-place live retuning (place/remove suppressed); only variant-capable blocks bind (a cube flashes invalid). Reuses the 125 orange highlight + "EDITING" title + `EditingInstance` propagation filter. Exits via the button / `E` / leaving build mode.](126-edit-block-button.md) |
| 125 | [Playtest follow-ups. **Garage opens in build mode when returning from a match** (drive mode still on fresh boot) via `GameStateController.ReturningFromArena`. **Rotor RPM became a throttle**: the slider is now Max RPM and live spin = throttle × max; a vertical-axis rotor trims on the vertical input (space/descend), a forward-axis prop on W/S, and the throttle HOLDS on release so a heli hovers hands-off (lift ∝ RPM² = real collective). Default throttle 1 preserves preset/AI behavior; `RpmOverride` still bypasses. **Rotor blades cone uniformly**: `UniformizeBladeTeeter` applies one teeter to all adopted blades (each blade's stored teeter was per-mount-face normalized → inconsistent); free wings keep mirror-V parity, so the two cases stay separate. **Instance-edit**: middle-click a placed block to retune just that one live (`BuildSession.EditingInstance`, propagation filtered to it), orange highlight + "EDITING" title, exit by re-click / empty-click / place / type-switch — no delete, no orphaning. Mortar-under-rotor parked (occlusion diagnosis noted). EditMode 341/342, PlayMode 114/115.](125-rotor-throttle-instance-edit.md) |
| 124 | [Closed the `BlueprintEntryTransform.Apply` gap (session-123 known unknown): `BlockConfig`, `ConcoctionId`, and `Yaw` now route through `IBlueprintEntryTransform` instead of defaulting to 0/""/0 on the transformed side. Config + concoction are orientation-free copies; yaw gets a real geometric reflection rule (`BlockMirror.MirrorYaw`): yaw' = baseOffset − yaw, where the 180° base offset appears exactly when the mirror flips `OrientationFromUp`'s deterministic forward-seed axis. Derivation pinned by 6 new `BlueprintEntryTransformTests`. No shipped preset layout changes — presets author via `ScriptedChassisBuilder`/session-mirror and can't set those fields inside a mirror; the fix closes the `BlueprintBuilder.Mirror` data-builder path. EditMode 341/342, PlayMode 114/115.](124-blueprint-entry-transform-yaw-config.md) |
| 123 | [Rotor RPM restored + RPM²-priced CPU, foil teeter axis + attachment-anchored pivot, middle-click picker, still garage sky. Per-rotor RPM slider (default 240 via new `RotorDefaults`; the P4 migration had left every rotor at the 60 RPM fallback ≈ 6% of advertised lift); CPU scales (RPM/240)² through `CpuBudget.EffectiveCpuCost` so lift-per-CPU is constant. Pitch visual moved to the span axis to match its physics (feathering); new visual-only `TeeterDeg` (schema v9) carries the old chord-axis tilt; both anchored at the attachment face via shared `ComputeWingPose` (ghost = placed mesh). Middle-click eyedropper picks a block's type + per-instance settings. Drive-by fixes: live-propagation pitch normalization, RepairPad config restore, live HUD concoction surcharge. EditMode 334/335, PlayMode 114/115.](123-rotor-rpm-teeter-picker.md) |
| 122 | [White procedural starfield](122-white-stars.md) |
| 121 | [Garage texture + liveliness pass](121-garage-liveliness.md) |
| 120 | [Playtest pass (3 categories, single session)](120-playtest-pass.md) |
| 119 | [Arena terrain: "Sunken Crossing" (diggable hills/ridges/valleys)](119-arena-terrain-sunken-crossing.md) |
| 118 | [Art & UI pass, part 2 (outlines + migrations + menu restyle)](118-ui-art-pass-2.md) |
| 117 | [UI theme consolidation (UGUI palette + HUD tone pass)](117-ui-theme-consolidation.md) |
| 116 | [Nanite Pulse: field self-repair module (new gameplay axis)](116-nanite-pulse-repair-module.md) |
| 115 | [Explosive knockback actually lands (self + enemies)](115-explosive-knockback-fix.md) |
| 114 | [Laboratory: player-authored explosive concoctions (ADR-0004)](114-laboratory-concoctions.md) |
| 113 | [Scrap deposit: warmup indicator + silent-loss fix](113-scrap-warmup-indicator.md) |
| 112 | [Weapon-fork refactor: phases C, D, A, B (ADR-0003)](112-weapon-fork-phases-cdab.md) |
| 111 | [CSP replay isolated in a prediction PhysicsScene (audit #1)](111-prediction-scene-csp.md) |
| 110 | [Audit remediation queue (carry-over)](110-audit-remediation-queue.md) |
| 109 | [<!-- Generated by a multi-agent full-app audit (10 finders + red-team + synthesis), 2026-06-03. Tier 3 reference: a point-in-time review, not a directive. -->](109-full-app-code-review.md) |
| 108 | [Combat pass: knockback, mortar, mines](108-combat-pass-knockback-mortar-mines.md) |
| 107 | [Movement consistency (/ideate round) — two shipped, one found-already-done. **CoM/CoL/CoT garage overlays** (`CenterOverlay`, toggle G): three translucent world-space spheres mark Center of Mass (white, live rb.worldCenterOfMass), Center of Lift (blue, area-weighted aero) and Center of Thrust (orange, thrust-weighted thrusters/hover/rotors); the spatial mismatch makes the session-106 wing physics legible. Read-only gizmo via RuntimeMaterials, collider-less, garage-only, wired by GarageController. **Hover blade mass/inertia scaling**: extends session-106's EffectiveMass + BlockInertiaBounds to hover blades (mass ∝ N² footprint, anchored at default size 2), so hover-vs-wheel is a real mass tradeoff and a size-4 pad weighs 4× a size-2 (lift already scaled N², so lift-per-mass stays honest — no free hover). **Thrust-offset torque** was approved then found already implemented (ThrusterBlock + aero/hover/rudder/wheel already AddForceAtPosition at the block's world position); no code change, the new CoT overlay visualises it. EditMode 271/272, PlayMode 111/112, 0 failures.](107-movement-consistency.md) |
| 106 | [Wing physics — bigger wings now weigh more and roll slower. Two fixes to a playtest finding that wing size had zero effect on roll. **B:** `Robot.RecalculateAggregates` gives aero blocks a mass that scales with foil volume (span·thickness·chord, anchored so default dims == Definition.Mass) and `ComputeDiagonalInertiaTensor` now sums a per-block solid-box inertia from each block's chassis-frame half-extents (`BlockOccupancy.ComputeSweptBoundsLocal`) — a non-aero block reduces exactly to the old `cellSize` cube so non-aero chassis are byte-identical, while a wing's real span feeds `Izz` (roll inertia ∝ span²). **A:** `PlaneControlSubsystem` scales pitch/roll/yaw authority by `clamp(refInertia/actualInertia, 0.3, 2.5)` (still ForceMode.Acceleration) so a higher-inertia (bigger-winged) plane rolls slower; at the reference inertia the default plane is unchanged. New `PlaneTuningConfig.{Pitch,Yaw,Roll}RefInertia` defaults = the measured DefaultPlane tensor (105/128/71 kg·m²). Measured payoff: default Izz 71 (roll ×0.99) → ×1.5 span Izz 179 (×0.40) → ×2 span Izz 328 (×0.30 floor). New PlayMode WingInertiaTests. No asset bake needed (ref fields default-initialise on load).](106-wing-physics.md) |
| 105 | [Modules — unified the single-slot active-module system into a multi-slot, per-block, tweakable ability system. A Module is now a block whose type IS its ability (`ModuleKinds` maps id↔kind); a chassis carries ≤4 (`ModuleBudget`), each on its own server-authoritative cooldown, surfaced on a MOBA ability bar (keys 1–4, new `ModuleBarHud`). Per-block power slider trades power for cooldown via `ModuleTuning` (`cooldown = base × clamp(power/default,0.5,2)`), riding the existing ConfigValue→BlockConfig→spawn path. Spring converted from the Space-hop Movement block to a grounded-only 10s-cooldown module (default 70 N·s, ½ the old 140). New Smoke (visual cloud + healthbar hide, no stat) and Invisibility (`StealthVisual` ~6% alpha + healthbar hide, ends on timer or 5% HP damage). EMP/Blink/Shield folded in. `IInputSource.ModulePressed`→`GetModulePressed(int slot)`; `InputCommand` wire bump to a `byte ModuleMask`. Blueprint schema v5→v6 (drops chassis `ActiveModuleKind`). 4-module cap at placement (`PlacementError.ModuleLimitReached`) + spawn-trim. Retired `ActiveModuleSystem`/`ActiveModuleBlock`/`ModuleDefinition`/`SpringBlock`/`ModuleSelectHud`/`ModuleHud`. New VFX `SmokeCloud`/`CloakShimmer` + audio `SmokeDeploy`/`Cloak`. ModuleDataTests/ModuleSystemTests/serializer v6 green. **Run "Robogame → Build Everything" to bake the six module block defs + library + presets + scenes (clears the two `block.module.active` preset failures).**](105-modules.md) |
| 104 | [Spring block (jump) + modular spring physics](104-spring-block.md) |
| 103 | [Fix: ClampBedrock counted the watertight boundary shell](103-clampbedrock-boundary-skip.md) |
| 102 | [CPU budget: spawn-time enforcement + garage fill bar](102-cpu-budget-enforcement.md) |
| 101 | [Active Module Slot (EMP / Blink / Disc Shield)](101-active-module-slot.md) |
| 100 | [Comfort pass — quieter thruster ignite/shutdown (8-bit sine beeps at vol 0.25 vs the 12-semitone CHARGE swooshes at vol 0.55/0.50 + hysteresis widened 0.55/0.45 → 0.85/0.15 so multi-thruster bombers stop chattering near mid-throttle). Shallower bomb craters — same SphereSubtract brush but centre is lifted along -gravity by 0.6×R, so the sphere bites a shallow dish (~0.4R deep) instead of a deep bowl, on flat AND spherical arenas. Bedrock + extended dig zone — `DigZone._bedrockCells` (default 3) seeds the bottom cell layers solid in `InitializeHeightmapSurface` and a new `ClampBedrock(chunk)` re-clamps them after every `ApplyBrush` / `ApplyBrushDeferred` on touched chunks only; arena dig zone widened from 6×1×6 (192 m) at (-96, -16, -96) to 7×1×7 (224 m) at (-112, -16, -112) so the ±100 m wall ring sits comfortably inside the diggable band (the old footprint left a 4 m strip of non-dig terrain at the walls). Drill `_digTargetSpeed` 2.0→2.6 m/s, `_glideTurnSpeed` 270→360 deg/s for snappier tunnelling. New PlayMode test `Bedrock_BottomCellsStaySolid_AcrossBrushes`. **Run "Robogame → Scaffold → Build Bootstrap (Pass A)" to regenerate the live Arena.unity DigZone with the new 7×1×7 / bedrock-3 config — scene asset still has the old 6×1×6 baked in.**](100-comfort-pass-bedrock-shallow-craters.md) |
| 99 | [Hover blade v1 — Robogame's first non-joint propulsion block. New `HoverBladeBlock` (in Robogame.Movement): raycast along `GravityField.SampleAt()` so flat + spherical arenas share one path; spring-damper with the spring clamped ≥ 0 (no-stratosphere — blade can't propel past target altitude) and damping gated to active spring (no drag-when-above-range surprise); force applied via `chassisRb.AddForceAtPosition` at attach point so banking, auto-leveling, and per-corner failure fall out for free with no explicit torque code. Variable footprint N×N×1 cells with N ∈ {2,3,4} via the aerofoil-style variable-dim pattern (Dims.x stores integer size, BlockOccupancy.ComputeHoverBladeSweptBoundsLocal computes the AABB perpendicular to mount-up). Lift and damping scale with N²; mass and CPU don't (constant 10 kg / 50 CPU — Robot.RecalculateAggregates reads `Definition.Mass` with no per-instance hook today, documented as v2 followup). VariantConfigPanel new hover-section with integer 2-4 slider + lift-multiplier readout. Dev tuning overrides for SpringK / DampingC / TargetAltitude via the session-98 pattern (compile-stripped from shipping builds). Looped HoverBladeLoop audio + persistent dust ParticleSystem child (ThrusterBlock pattern; toggle emission rate not spawn-per-frame to stay within VfxSpawner budget). New Blueprint_DefaultHoverTank preset (5×5 chassis structure, 4 corner size-2 blades — wheels replaced) wired as preset slot 8 in GameplayScaffolder. 4 new PlayMode tests verify the load-bearing invariants (no-stratosphere, ray-miss fall, N² scaling, destroyed-blade silence). New physics.md § 2.1 documents the raycast spring-damper pattern as the model for future non-joint propulsion blocks. **Run "Robogame → Scaffold → Build Bootstrap (Pass A)" in editor to populate the new preset in Bootstrap.unity**](99-hoverblade-v1.md) |
| 98 | [Dev-only aircraft tuning overrides. Restores the pre-session-85 pitch / roll / thrust / damping sliders to the in-game Settings panel as a compile-stripped dev-only override layer. New DevTuningOverride helper (lives in Robogame.Block so it can mutate PlaneTuningConfig/GroundTuningConfig/ChassisDampingConfig/ThrusterTuningConfig — Core can't reference Block). 13 new Tweakables registered inside #if UNITY_EDITOR \|\| DEVELOPMENT_BUILD: master Dev.OverrideChassisTuning toggle + 6 plane knobs (PitchPower, RollPower, YawFromBank, PitchDamping, RollDamping, YawDamping) + 3 ground (Acceleration, MaxSpeed, TurnRate) + 2 chassis damping + 2 thruster (IdleThrottle, ThrottleResponse). Defaults match the historical pre-migration Tweakable defaults so toggling on with sliders untouched preserves the old feel. Consumers (PlaneControlSubsystem, GroundDriveSubsystem, ThrusterBlock, RobotDrive) extracted into ResolveTuning() / PushChassisDamping() that read blueprint then call DevTuningOverride.Apply\*; subscribe to Tweakables.Changed so sliders take effect live without a respawn. Invariant #1 (no Tweakable affects gameplay outcomes) is preserved because the entire override surface — Tweakable registration, helper methods, consumer subscriptions — compile-strips to nothing in shipping builds. EditMode 252/253, PlayMode 92/93 green](98-dev-tuning-override-aircraft-tweaks.md) |
| 97 | [Slice D — Phase 7 QoL SP layer: scoreboard + kill feed + nameplates. New KillFeedHud: ring-buffer (6 entries default) of recent kills on the top-right, subscribes to MatchController.KillRegistered for BOTH player and enemy kills (unlike KillAnnouncer which is player-only), holds full opacity for 5 s then fades over 0.8 s; entries tint green for player kills / red for enemy / muted for environment. New NameplateOverlay: one camera-scoped OnGUI walks a periodically-refreshed Robot list (FindObjectsByType every 1 s) and renders a name label + HP bar above every non-local chassis, culled past 120 m, HP-bar palette matches ObjectiveHud's Healthy/Warning/Danger. New ScoreboardOverlay: tab-held centred overlay with two-column per-side state (scrap totals, frag counts, lives) and a centred timer row; reads MatchController directly so no caching needed. All three auto-attached in ArenaController.ConfigureCamera. NGO replication sibling deliberately deferred — user is a netcode beginner and placeholder NetworkBehaviour scaffolding without an exercising flow would create dead surface; the SP HUDs already read through the right abstractions (MatchController event + per-Robot state) so the future Phase 7 MP push fills in the network sibling without changing the HUDs. EditMode 252/253, PlayMode 92/93 green throughout](97-slice-d-scoreboard-killfeed-nameplates.md) |
